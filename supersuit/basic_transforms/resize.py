...
def check_param(obs_space, resize):
    xsize, ysize, linear_interp, _ = resize
...
def change_observation(obs, obs_space, resize):
    xsize, ysize, linear_interp, crop = resize
    if crop:
        obs = obs[25:185, 20:140, :]  # NOTE These cropping parameters are tuned for Boxing domain
    if len(obs.shape) == 2:
